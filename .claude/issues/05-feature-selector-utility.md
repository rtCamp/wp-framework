...
- [2026-05-07] **Settings page now uses the WordPress Settings API end-to-end** (review feedback from @bhavz-10). The first pass used the *persistence* half of the API (`register_setting` + `settings_fields` + form action `options.php`) but hand-rolled the rendering as a manual `<table class="form-table">` loop. Refactored to use `add_settings_section` + `add_settings_field` per flag with a `render_field()` callback, and replaced the manual table with `do_settings_sections( $page_slug )`. Same HTML output, same persistence behaviour, but the rendering now goes through the canonical Settings API path so a reviewer reading the file sees the standard pattern. One section (`rtcamp_features_section`) holds every field; field titles get the human `name`, the callback renders the checkbox + optional description + optional constant-lock notice.
...
- `src/Utilities/Feature_Selector_Settings_Page.php` — new (admin page renderer). Uses the Settings API end-to-end (`add_settings_section` + `add_settings_field` per flag + `do_settings_sections` for output). Renders `name` + `description`, and when the override constant is defined the checkbox is disabled, mirrors the constant's value, and a help message names the constant.
...
- **Reviewer checklist**:
  ...
  - Settings registered and rendered via the WP Settings API end-to-end — `register_setting` + `add_settings_section` + `add_settings_field` for wiring; `settings_fields` + `do_settings_sections` + `submit_button` + form action `options.php` for output. No custom POST handling, no hand-rolled nonce, no manual `<table class="form-table">` loop.
