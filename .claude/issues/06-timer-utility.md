...
- [2026-05-05] **`stop()` on already-stopped returns the cached elapsed.** ~~Originally silent~~ — as of 2026-05-13 also fires `_doing_it_wrong()`. Callers that need to stop conditionally from multiple paths should guard the call rather than double-stopping.
...
- [2026-05-13] **`_doing_it_wrong()` added to all silent no-op paths per PR review.** `start('')`, `start()` on already-started label, `stop('')`, and `stop()` on already-stopped label now all fire `_doing_it_wrong()`. Tests updated with `setExpectedIncorrectUsage()`. Docblocks updated to document the warning on each path.
...
- Every misuse path (`start`, `stop`, `lap`) now fires `_doing_it_wrong()` — no silent no-ops remain except `lap('', ...)` and `get('')` which return without side-effects.
